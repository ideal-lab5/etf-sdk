...
use etf_crypto_primitives::{
    ...
    tlock::tlock::{tle as timelock_encrypt, DecryptionResult, TLECiphertext},
    utils::{self, *},
};
...
pub fn tle(
    ...
    let double_pub_key = convert_from_bytes::<DoublePublicKey<TinyBLS377>, 144>(&pp_bytes.clone())
    ...
    let ciphertext: TLECiphertext<TinyBLS377> =
        timelock_encrypt(pp, msk_bytes, &message_bytes, identity, rng)
...
pub fn tld(ciphertext_js: JsValue, sig_vec_js: JsValue) -> Result<JsValue, JsError> {
    ...
    let decrypt_result: DecryptionResult = ciphertext
        .tld(sig_point)
...
#[wasm_bindgen]
pub fn aes_decrypt(ciphertext_js: JsValue, sig_vec_js: JsValue) -> Result<JsValue, JsError> {
    let sig_conversion: Vec<u8> = serde_wasm_bindgen::from_value(sig_vec_js.clone())
        .map_err(|_| JsError::new("could not decode secret key"))?;
...
pub fn build_encoded_commitment(
    ...
    let block_number: u32 = serde_wasm_bindgen::from_value(block_number_js.clone())
...
pub fn extract_signature(id: JsValue, sk_js: JsValue) -> Result<JsValue, JsError> {
    let sk: [u8; 32] =
        serde_wasm_bindgen::from_value(sk_js).map_err(|_| JsError::new("Could not sk to array"))?;
    let msk = convert_from_bytes::<<TinyBLS377 as EngineBLS>::Scalar, 32>(&sk.clone()).unwrap();
    let identity_vec: Vec<u8> = serde_wasm_bindgen::from_value(id)
            ...
            match tle(identity_js, message_js, sk_js, p_pub_js) {
                Ok(ciphertext) => {
                    ...
                    match tld(ciphertext_clone, sig_vec_js) {
                        ...
                        Err(error) => handler(TestStatusReport::DecryptFailure { error }),
                    }
                }
